# --- main.py（ver.2025.08f：確実反映＋色同期安定版）---
...
# 関数
# -------------------------------------------------------------
def parse_time(t): h,m = map(int,t.split(":")); return time(h,m)
...
def sync_full_room(date):
    ...
    for f in fwd:
        for b in bwd:
            if f["date"]==b["date"]==date and overlap(parse_time(f["start"]),parse_time(f["end"]),
...
def register_reservation(room,date,start,end,user,purpose,ext):
    new={"date":date,"start":start,"end":end,"user":user,"purpose":purpose,"extension":ext}
...
# -------------------------------------------------------------
    ...
    with st.form("add"):
        ...
        with c1: room=st.selectbox("区画",ROOMS)
        with c2: start=st.select_slider("開始",options=TIME_SLOTS,value="12:00")
        with c3: end=st.select_slider("終了",options=TIME_SLOTS,value="13:00")
        with c4: user=st.text_input("氏名",max_chars=16)
        with c5: purp=st.text_input("目的",placeholder="任意")
        with c6: ext=st.text_input("内線",placeholder="例：1234")
        sub=st.form_submit_button("登録")
        if sub:
            s=parse_time(start); e=parse_time(end)
            if e<=s: st.error("終了時刻は開始より後に。")
            elif not user.strip(): st.warning("氏名を入力してください。")
            else: register_reservation(room,date,start,end,user,purp,ext)
    ...
    for rname,items in st.session_state["reservations"].items():
        for it in items:
            if it["date"]==date: allres.append({"room":rname,**it})
    if allres:
        df=pd.DataFrame(allres)
        sel=st.selectbox("削除対象",df.apply(lambda x:f\"{x['room']} | {x['user']} | {x['start']}〜{x['end']}\",axis=1))
        if st.button("選択した予約を取消"):
