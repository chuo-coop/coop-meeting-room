        ...
        for rname in ["前方区画", "後方区画"]:
            for r in st.session_state["reservations"][rname]:
                if (r["date"] == date) and overlap(parse_time(r["start"]), parse_time(r["end"]), parse_time(start), parse_time(end)):
                    st.warning(f"{rname} に既に予約があります。全面利用はできません。")
                    return False
        for rname in ["前方区画", "後方区画"]:
            st.session_state["reservations"][rname].append(new_res)
        return True
    else:
        for r in st.session_state["reservations"]["全面利用"] if "全面利用" in st.session_state["reservations"] else []:
            if (r["date"] == date) and overlap(parse_time(r["start"]), parse_time(r["end"]), parse_time(start), parse_time(end)):
                st.warning("この時間帯は全面利用で予約済みです。")
                return False
...
def cancel_reservation(room, user, start, end, date):
    targets = ["前方区画", "後方区画"] if room == "全面利用" else [room]
    for t in targets:
...
if st.session_state["page"] == "calendar":
    ...
    if any(r["date"] == selected_date for r in st.session_state["reservations"]["前方区画"]) and any(r["date"] == selected_date for r in st.session_state["reservations"]["後方区画"]):
        st.markdown("<div style='background:#ff6666;color:white;text-align:center;padding:6px;font-weight:bold;'>全面利用中（満）</div>", unsafe_allow_html=True)
    ...
    st.caption("中央大学生活協同組合　情報通信チーム（v3.4.4＋ 完全構造維持版）")
